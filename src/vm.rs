...
mod test {
    ...
    fn test_eval_context() {
        let multiply = r#"
            ...
            fn multiply(number: u64, n: u64, add: bool) -> u64 {
                if add {
                    var acc = 0;
                    while n > 0 {
                        acc = acc + number;
            ...
            fn main() -> bool {
                return multiply(10, 5, false) == multiply(10, 5, true);
            }
        "#;
        ...
        let fibonacci_10 = r#"
            ...
            fn main() -> u64 {
                return fib(30); // 30 takes a while, but is not too large
            }
        "#;
        ...
        let scoping = r#"
            fn main() -> u64 {
                ...
                return x; // Should return 10, not 30
            }
        "#;

        let shortcut = r#"
            var do_not_touch: u64 = 0;
            fn main() -> u64 {
                true || increment();
                false || increment();
                false && increment();
                ...
                return do_not_touch; // Should return 2, not 4
            }
        ...
        let call_undefined = r#"
            fn main() -> u64 {
                // Rust code should handle `UnknownFunctionCall` event and provide a return value.
                return 2 * not_defined(2);
            }
        "#;

        let test_cases = [
            (multiply, Value::Bool(true)),
            (fibonacci_10, Value::U64(832040)),
            (globals, Value::U64(52)),
            (shortcut, Value::U64(2)),
            (scoping, Value::U64(10)),
            (call_undefined, Value::U64(20)),
        ];
        for (source_code, expected) in test_cases {
                ...
                match context.run() {
                    EvalEvent::UnknownFunctionCall => {
                        ...
                        if name == "not_defined" {
                            match args[0] {
                                Value::U64(a) => {
                                    context.set_return_value(Value::U64(a * 5)).unwrap()
                                }
                                _ => {}
                            }
                        }
